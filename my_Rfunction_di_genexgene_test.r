CAST<-function(i,j){
  ...
  res<-cbind(genename1= gene[i][1], genename2= gene.rare[j][1], LD= dist, sample.size = length(dom1),
...
analysis<-function(outfile=NULL,Gnomad_AF1=0.2,Gnomad_AF2=0.0001,AAF1=1,AAF2=1,set1=c("frameshift","stop_gained","stop_lost","start_lost","splice_donor","splice_acceptor"),set2="ALL",
  ...
  # Filter variants
  variant1 = which(Gnomad<=Gnomad_AF1 & !grepl("WARN",WARN))
  ...
  write.table(resultpar,file=paste(direct,"/",feno,"/di/pvalues/di_genexgene_Gnomad_AF1",Gnomad_AF1,"_Gnomad_AF2",Gnomad_AF2,"_AF1",AAF1,"_AF2",AAF2,"_",effects,"_vcf.out",sep=""),row.names=F,col.names=T,quote=F,sep="\t")
  stopCluster(cl)
  return(resultpar)
}